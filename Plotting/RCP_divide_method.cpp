...
int RCP_27GeV_pos(Int_t mode = 1, TString FileName = "/home/manukhov/Dubna/files/27GeV_19131.root")
{
//Getting information from the file --------------------------------------------------------------------------------
	...
	TH1D *hPos0_5;
	TH1D *hNeg0_5;
	File->GetObject("hPosSpectra_8", hPos0_5);
	File->GetObject("hNegSpectra_8", hNeg0_5);   
	TH1D* hCent9;
...
// Creating RCP histo -------------------------------------------------------------------------------------------------
	...
	hNegRCP = (TH1D*)hPos0_5->Clone("NegRCP");
...
// Drawing --------------------------------------------------------------------------------------------------------------
	...
	if(mode == 1) {
		hPosRCP->SetTitle("Positive R_{CP} destribution");
		hPosRCP->SetAxisRange(0.,3.5, "Y");
		hPosRCP->SetAxisRange(0.2,5, "X");	
		hPosRCP->Draw("e1p");
	
		hNegRCP->SetTitle("Negative R_{CP} destribution");
...
// Semisumm of RCPs -----------------------------------------------------------------------------------------------------
	if (mode == 2) {
		TH1D* hCharge;
		File->GetObject("hCharge", hCharge);   
		Double_t Nplus = hCharge->GetBinContent(3);
		Double_t Nminus = hCharge->GetBinContent(1);	
		for(Int_t i = 0; i < NumBins; i++) {
			Double_t content = (Nplus*hPosRCP->GetBinContent(i) + Nminus*hNegRCP->GetBinContent(i))/(Nplus+Nminus);
			hPosRCP->SetBinContent(i, content);
			hPosRCP->SetBinError(i, ( sqrt( pow(hPosRCP->GetBinError(i),2)+pow(hNegRCP->GetBinError(i),2)  )/2        ));
		}
		hPosRCP->SetMarkerColor(1);
		hPosRCP->SetTitle("Average R_{CP} destribution");
		hPosRCP->SetAxisRange(0.,3.5, "Y");
		hPosRCP->SetAxisRange(0.2,5, "X");	
		hPosRCP->Draw("e1p");
	}
